...
# Read and preserve prior MSR shortcut identities before replacing those shortcuts.
# The launcher never imports FN databases and never silently replaces an existing profile.
$profilePreparation = Start-Process -FilePath $launcher -ArgumentList @('--prepare-profile', '--desktop', ('"' + $Destination + '"')) -Wait -PassThru
if ($profilePreparation.ExitCode -ne 0) { throw 'Profile preparation was cancelled or failed. Existing shortcuts were left unchanged.' }
$entries = @(
