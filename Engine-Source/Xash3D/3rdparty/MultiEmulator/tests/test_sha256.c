...
int main( int argc, char **argv )
{
	...
	CSHA sha;
	CSHA_Reset( &sha );
	CSHA_AddData( &sha, "abc", 3 );
	CSHA_FinalDigest( &sha, result );
	...
	CSHA_Reset( &sha );
	CSHA_AddData( &sha, "abcdbcdecdefdefgefghfghighijhijkijkljklmklmnlmnomnopnopq", 56 );
	CSHA_FinalDigest( &sha, result );
	...
	CSHA_Reset( &sha );
	CSHA_AddData( &sha, "abcdefghbcdefghicdefghijdefghijkefghijklfghijklmghijklmnhijklmnoijklmnopjklmnopqklmnopqrlmnopqrsmnopqrstnopqrstu", 112 );
	CSHA_FinalDigest( &sha, result );
