...
void ComputeTexGen( int i )
{
	    ...
	    || uTexGenMode[i].z == GEN_REFLECTMAP )
	{
		...
		if( uTexGenMode[i].x == GEN_SPHEREMAP )
		{
			vTexCoord[i].x = ( r.x * m ) + 0.5;
		}
		if( uTexGenMode[i].y == GEN_SPHEREMAP )
		{
			vTexCoord[i].y = ( r.y * m ) + 0.5;
		}
		if( uTexGenMode[i].x == GEN_REFLECTMAP )
		{
			vTexCoord[i].x = r.x;
		}
		if( uTexGenMode[i].y == GEN_REFLECTMAP )
		{
			vTexCoord[i].y = r.y;
		}
		if( uTexGenMode[i].z == GEN_REFLECTMAP )
		{
			vTexCoord[i].z = r.z;
		}
	}
