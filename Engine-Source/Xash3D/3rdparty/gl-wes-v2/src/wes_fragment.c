...
wes_frag_arg_obtain( char *buff, progstate_t *s, int tex, int arg, int type )
{
	...
	switch( type )
	{
	case 4: strcpy( comp, "rgba" );
		src = s->uTexture[tex].Arg[arg].RGBSrc;
		break;
	case 3: strcpy( comp, "rgb" );
		src = s->uTexture[tex].Arg[arg].RGBSrc;
		break;
	case 1: strcpy( comp, "a" );
		src = s->uTexture[tex].Arg[arg].AlphaSrc;
		break;
	default: PRINT_ERROR( "Unknown Argument Type: %i", type );
		break;
	}
	...
	default:    PRINT_ERROR( "ERROR: No valid SRC for TEX%i ARG%i.%s", tex, arg, comp );
		break;
	}
	return (GLint)( str - buff );
};
...
wes_frag_combine( char *buff, progstate_t *s, int tex )
{
	...
	if( s->uTexture[tex].RGBCombine == s->uTexture[tex].AlphaCombine )
	{
		...
		default:    PRINT_ERROR( "ERROR: No valid FUNC for TEX%i", tex );
			break;
	...
	else
	{
		...
		default:    PRINT_ERROR( "ERROR: No valid FUNC for TEX%i.rgb", tex );
			break;
		}
		...
		default:    PRINT_ERROR( "ERROR: No valid FUNC for TEX%i.a", tex );
			break;
		}
...
wes_frag_tex( char *buff, progstate_t *s )
{
	...
	for( i = 0; i < WES_MULTITEX_NUM; i++ )
	{
		if( s->uTexture[i].Enable )
		{
			...
			default:    PRINT_ERROR( "ERROR: No valid MODE for TEX%i", i );
				break;
			}
		}
	}
