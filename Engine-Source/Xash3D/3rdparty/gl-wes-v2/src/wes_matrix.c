...
GL_MANGLE( glMatrixMode )( GLenum mode )
{
	...
	switch( m_mode )
	{
	case GL_MODELVIEW:  m_current = m_modelview;
		break;
	case GL_PROJECTION: m_current = m_projection;
		break;
	case GL_TEXTURE:    m_current = &m_texture[u_activetex * WES_TEXTURE_NUM];
		break;
	case GL_COLOR:      m_current = m_color;
		break;
	default:            break;
	}
}
...
GLvoid
gluLookAt( GLfloat eyex, GLfloat eyey, GLfloat eyez, GLfloat centerx, GLfloat centery, GLfloat centerz, GLfloat upx, GLfloat upy, GLfloat upz )
{
