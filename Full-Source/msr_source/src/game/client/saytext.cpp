...
// Screen dimensions are not initialized when this DLL's globals are created.
...
{
	int y = ScreenHeight() - YRES(160) - (line_height * (MAX_LINES + 1));
...
void CHudSayText :: SayTextPrint( const char *pszBuf, int iBufSize, int clientIndex )
{
	...
	const int copyLength = V_min(iBufSize - 1, MAX_CHARS_PER_LINE - 1);
	strncpy(g_szLineBuffer[i], pszBuf, copyLength);
	g_szLineBuffer[i][copyLength] = '\0';
...
void CHudSayText :: EnsureTextFitsInOneLineAndWrapIfHaveTo( int line )
{
	if (line < 0 || line >= MAX_LINES)
		return;
	const int maxLineWidth = ScreenWidth() - 40;
	// Defer wrapping until video/font initialization provides a usable width.
	if (maxLineWidth <= LINE_START)
		return;
	int line_width = 0;
	GetConsoleStringSize( g_szLineBuffer[line], &line_width, &line_height );

	if ( (line_width + LINE_START) > maxLineWidth )
	{ // string is too long to fit on line
		...
		for ( char *x = g_szLineBuffer[line]; *x != 0; x++ )
		{
			...
			{  // needs to be broken up
				if (!last_break)
					last_break = x > g_szLineBuffer[line] ? x : x + 1;

				// Own the remainder before scrolling can move or discard this row.
				// Every wrap consumes at least one character, including at tiny widths.
				char remainder[MAX_CHARS_PER_LINE];
				strncpy(remainder, last_break, sizeof(remainder) - 1);
				remainder[sizeof(remainder) - 1] = '\0';
				*last_break = '\0';
				const char *nextText = remainder;
				...
				strncpy(g_szLineBuffer[j], nextText, MAX_CHARS_PER_LINE - 1);
				g_szLineBuffer[j][MAX_CHARS_PER_LINE - 1] = '\0';
				g_iNameLengths[j] = 0;
				g_pflNameColors[j] = NULL;
