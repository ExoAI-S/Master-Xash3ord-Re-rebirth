...
# Работа с текстурами
PrimeXT поддерживает как минимум два формата текстур, и у каждого формата есть свои особенности при использовании.
...
- **`.dds`** - Формат, поддерживающий прозрачность и сжатие с потерями (lossy compression). При неправильной настройке возможно появление артефактов сжатия на текстуре при экспортировании (обычно в виде квадратов). 
...
## Parallax Mapping
Технология, позволяющая смоделировать объёмный рельеф на поверхности, не изменяя при этом её геометрию. В PrimeXT может применяться для брашей, моделей и декалей.
...
## Gloss/PBR текстура
Данная текстура хранит в себе различные параметры, которые влияют на итоговый внешний вид объекта, на котором использована эта текстура. Для данного типа текстур можно использовать суффикс **`_gloss`** или **`_pbr`**. Более подробное описание этих параметров приведено ниже.
...
:::tip Примечание
На данный момент, реализация PBR пайплайна ещё не завершена, из этого следует что в будущем возможны изменения, сильно влияющие на визуальный вид контента, созданного под PBR пайплайн. Если важна стабильность и вы не хотите переделывать весь контент в будущем - то лучше пока что не использовать PBR.
:::
...
## Cветящиеся текстуры (luma/emissive)
...
он позволяет задавать значения цвета в широком динамическом диапазоне (HDR), что значительно расширяет возможности luma-текстур. Например, детали можно сделать намного более яркими, чем если бы использовалась текстура в обычном динамическом диапазоне.
...
Даже если вы используете внешнюю текстуру с альфа-каналом, по умолчанию прозрачность полностью отключена. Для того чтобы её активировать, нужно выполнить дополнительные действия:

### Студиомодели
В нужном MDL-файле при помощи PrimeXT Model Viewer (либо в QC-файле на этапе компиляции модели) требуется выставить определённые флаги для желаемой текстуры:

- `Transparent`, нет внешней текстуры: будет использоваться альфа-тест (тексель либо полностью непрозрачный, либо полностью прозрачный)
- `Transparent`, внешняя текстура с альфа-каналом: будет использоваться альфа-блендинг (прозрачность с плавными переходами)
- `Transparent` + `Alpha To Coverage`, внешняя текстура с альфа-каналом: будет использоваться альфа-тест c дополнительным сглаживанием краёв (alpha to coverage)

:::tip Примечание
Выбор подходящего режима прозрачности напрямую зависит от геометрии объекта и желаемого результата. Альфа-блендинг работает корректно только на плоских (окна, платформы), либо выпуклых объектах (сферы, цилинды, кубы и другие простые геометрические объекты). В остальных случаях, для любой другой геометрии, имеющей вогнутости или самопересечения (например листва, трава или деревья), рекомендуется использовать режим alpha to coverage.
:::

### Браши
Для использования прозрачности, в настройках брашевой энтити, в пункте **`Render Mode`** указываем значение **`Texture`** или **`Additive`**. Также, прописываем уровень прозрачности в **`FX Amount`**. Либо сама текстура в WAD должна иметь синий канал для прозрачности.
...
Список суффиксов: `bk`, `dn`, `ft`, `lf`, `rt`, `up`. Эти текстуры располагаются в папке **`gfx\env`** и могут быть в форматах **DDS** или **TGA**.

Имеется возможность использования HDR-скайбокса, для этого нужно упаковать HDR-текстуры скайбокса в формат DDS со сжатием BC6H.
