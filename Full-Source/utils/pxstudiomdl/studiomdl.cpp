...
static vfile_t *GenerateModelScript(const char *modelName)
{
	...
	if (file)
	{
		VFS_Printf( file, "// Autogenerated QC-Script.\n" );
		VFS_Printf( file, "\n$modelname \"%s.mdl\"", modelName );
		VFS_Printf( file, "\n$cd \".\\\"" );
		VFS_Printf( file, "\n$cdtexture \".\\\"" );
		VFS_Printf( file, "\n$scale 1.0" );
		VFS_Printf( file, "\n$cliptotextures\n" );
...
int main( int argc, char **argv )
{
	...
	{	
		...
		// this buffer will be deallocated automatically in EndOfScript()
		byte *tempBuffer = reinterpret_cast<byte*>(Mem_Alloc(file->length));
		memcpy(tempBuffer, file->buff, file->length);
		ParseFromMemory(reinterpret_cast<const char*>(tempBuffer), file->length);
		VFS_Close(file);
	}
	else {
		LoadScriptFile(path);
